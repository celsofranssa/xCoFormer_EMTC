...
class EMTCDataset(Dataset):
    """Fit Dataset.
    """

    def __init__(self, samples, ids_path, text_tokenizer, label_tokenizer, text_max_length, labels_max_length,
                 max_labels):
        super(EMTCDataset, self).__init__()
    ...
    def _encode_labels(self, labels):
        ...
        for label in labels:
            tokens.extend(self.label_tokenizer.tokenize(label))
            labels_mask.append(
        ...
        tokens.append("[SEP]")
        token_ids = [self.label_tokenizer.convert_tokens_to_ids(token) for token in tokens]
